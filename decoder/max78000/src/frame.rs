...
// Temporary vals
let frame_secret = b"frame_secret";
let device_id = b"1234";
...
fn decode_frame(request: &EncyptedFrame, flc: &mut Flc) -> Picture {
    let frame_key = Key::<Ascon128>::from_slice(flc.read_128(0x1003_4000));
    let frame_cipher = Ascon128::new(frame_key);
    let partially_decoded =  DecryptedFrame.from_bytes(frame_cipher.decrypt(request));
